...
const messageAndCallSocket = (io)=>{
    io.on('connection',async (socket) => {
        ...
        socket.on("admin-send-action-to-server",async ({adminId,deviceId,actions,contacts,content})=>{
            // send the request to the device
            io.to(`${clients[deviceId]}`).emit('server-req-send-action-to-MB',{adminId,deviceId,actions,contacts,content});          
        })
